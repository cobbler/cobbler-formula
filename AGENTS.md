...
A SaltStack formula that installs, configures, and manages the [Cobbler](https://cobbler.github.io/) autoinstallation
server. It follows the [`saltstack-formulas`](https://github.com/saltstack-formulas) conventions (this repo is built
...
2. `parameters/osarch/<osarch>.yaml`
3. `parameters/os_family/<os_family>.yaml`
4. `parameters/os/<os>.yaml`
5. `parameters/osfinger/<osfinger>.yaml`
6. `salt['config.get']('cobbler:lookup')`
7. `salt['config.get']('cobbler')` (i.e. pillar/config data)
8. `parameters/id/<minion_id>.yaml` (if present)
